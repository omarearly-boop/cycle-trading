...
FACTOR_PATTERNS = [
    ...
    (r'\bSSR\b|uptick|בוטלה.{0,20}פקוד|פקודה.{0,20}בוטלה|הגבלת שורט',
     'ssr', 'SSR_Risk flag — Rule 201 warning', True),
    (r'(?<![A-Za-z])POC(?![A-Za-z])|volume profile|רצפת בטון|פרופיל ווליום|פרופיל נפח',
     'volume_profile', 'Volume-Profile POC support', False),
    (r'השבוע רק החל|נר שבועי.{0,15}פתוח|שבוע לא נסגר|partial.{0,10}bar',
     'partial_bar', 'PartialBar flag — mid-week caution', True),
...
SCANNER_STATUS = {
    ...
    'stop_candle':        ('✅ קיים',  'OK',   '#22c55e', 'סטופ = min(מתחת לתמיכה, מתחת לנמוך הנר) — יושם יולי 2026'),
    'ssr':                ('✅ קיים',  'OK',   '#22c55e', 'דגל SSR_Risk על שורטים אחרי נפילה יומית ≥10%'),
    ...
    'volume_profile':     ('🔧 חסר',  'GAP',  '#f59e0b', 'POC/רצפת בטון — תמיכה לפי פרופיל נפח, לא מיושם'),
    'partial_bar':        ('✅ קיים',  'OK',   '#22c55e', 'דגל PartialBar — סריקה באמצע שבוע = נר שבועי פתוח'),
