...
def run_check():
    ...
    if _yf is not None:
        ...
        for _entry in active_entries:
            ...
            try:
                _hist = _yf.Ticker(_ticker).history(period='6mo', interval='1wk')
                if _hist.empty or len(_hist) < 6:
                    continue
            ...
            if _is_long:
                ...
                if _swings:
                    _new_stop = max(_swings[-3:])  # highest recent swing low
                ...
                if _swings:
                    _new_stop = min(_swings[-3:])  # lowest recent swing high
                    _trail_stored = _entry.get('trail_stop') or _stop_p
                    if _new_stop < _trail_stored and _new_stop > _cur_p:
